...
var RetryPolicy = require('../lib/RetryPolicy.js');
...
describe('RetryPolicy', function () {
	...
	it('merges user provided options with defaults', function () {
		...
		var emitter = RetryPolicy.enableRetryPolicy(mock.object, options);

		assert.strictEqual(options.maxRetries, 10);
		assert.strictEqual(options.retryCalculation, RetryPolicy.defaults.retryCalculation);
		assert.strictEqual(options.timeSlot, RetryPolicy.defaults.timeSlot);
		assert.strictEqual(options.maxDelay, RetryPolicy.defaults.maxDelay);
	});

	it('listens to flush ops starting on a bulkinsert instance', function () {
		var mock = createRSBLMock();

		var emitter = RetryPolicy.enableRetryPolicy(mock.object);
